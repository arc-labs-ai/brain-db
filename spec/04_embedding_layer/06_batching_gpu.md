# 04.06 Batching and GPU

The optional GPU inference path. This file specifies how the substrate batches multiple inference requests onto a GPU when one is available.
...
The substrate batches automatically when GPU is enabled. The batching happens at the embedding layer; clients are unaware.

## 3. The batching window

The substrate gathers in-flight inference requests over a small time window (default: **2 ms**). At the end of the window, all gathered requests are submitted as a single batch.
...
2 ms is the sweet spot for our target workloads. It's:
...
For workloads with many shards, this means GPU utilization is shared across shards. The substrate may also support a "global batcher" that pools requests from all shards onto a shared GPU; this is configuration option for deployments that want maximum GPU efficiency.
...
For multi-GPU systems, the substrate uses GPU 0 by default. Configuration can pin to a specific device:
...
Multi-GPU inference (sharding the model across GPUs) is not supported in v1. The model is small enough to fit on a single GPU; multi-GPU is unnecessary complexity.
...
The substrate doesn't currently sort items by length within a batch (a "sequence bucketing" optimization). It's a possible v2 improvement; for now, we accept the inefficiency.

## 13. Determinism

GPU inference may produce slightly different results than CPU (different floating-point order, different libraries). For our purposes, this is acceptable — cosine similarity is robust to small perturbations.
...
If GPU is not available or not configured, the substrate runs CPU-only. There is no batching on the CPU path; each request goes through the model independently. CPU inference scales by adding cores.
...
*Continue to [`07_fingerprinting.md`](07_fingerprinting.md) for the model fingerprint.*
