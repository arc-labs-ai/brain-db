# 04.09 Embedding Layer Failure Modes

What can go wrong in the embedding layer and how the substrate responds.

## 1. Model file missing or corrupted

**Failure mode.** At startup, the substrate can't find the model files at the configured path, or the files fail integrity checks.
...
**Response.** The substrate refuses to start. The error message identifies the failing file and the expected vs actual state.
...
**Response.** The encode operation fails with `EmbeddingNumericFailure`. The text is logged (truncated) for diagnostic. The substrate doesn't store the bad vector.
...
**Response.** The encode operation fails with `EmbeddingTimeout`. The substrate logs the event with the input text length.

**Operator action.** Investigate whether the substrate is overloaded (CPU saturated) or whether the model has a pathological case. Adjust the timeout if a deployment legitimately has long-running inference (very long sequences on slow hardware).
...
**Response.** The encode operation fails with `EmptyTextAfterTokenization`. The substrate suggests the text be cleaned or replaced.
...
**Failure mode.** The input text tokenizes to more than 512 tokens. Not actually a failure — the substrate truncates and proceeds.
...
- Periodically, the substrate retries GPU operations to see if the issue has resolved.
...
**Detection.** Implicit: the substrate validates norm on cached vectors infrequently. Most cache hits are not validated.

**Response.** None directly — the substrate trusts cached vectors. If a vector is bad, queries using it produce poor results until the cached entry is evicted.

**Mitigation.** Restart the substrate to clear the cache. Restart on schedule or on-demand if poor query quality is observed.
...
**Response.** The substrate fails to start with `ModelLoadTimeout`. The error message indicates the configured path and the time spent.
...
**Operator action.** Run `ADMIN_MIGRATE_EMBEDDINGS` to convert old-fingerprint memories to the new fingerprint. See [`08_migration.md`](08_migration.md).

## 12. Unsupported model file format

**Failure mode.** The model files are in a format the substrate doesn't support (e.g., an old PyTorch pickle that candle can't load, or a custom format).
...
**Response.** The substrate logs a warning ("loading pickle weights; this format can execute arbitrary code"). It still proceeds with the load. If `embedding.refuse_pickle = true` (a configuration option, default `false`), the substrate refuses to load pickle and fails startup with `PickleRefused`.
...
**Detection.** Per-shard queue depth. If a queue exceeds `embed_queue_max` (default 1000), the substrate is overloaded.
...
**Detection.** The substrate doesn't actively detect this. It would manifest as poor cache hit rates and subtle quality issues.
...
**Detection.** The substrate doesn't detect this directly. Symptoms include poor recall quality and unusual confidence calibration.
