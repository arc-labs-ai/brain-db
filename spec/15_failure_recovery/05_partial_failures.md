# 15.05 Partial Failure Handling
...
- The substrate may attempt to recover the shard (restart its tasks).
...
The substrate's health endpoint returns per-shard state.
...
The substrate can recover automatically; operator review confirms.
...
3. The substrate doesn't keep retrying restart.
4. Alert escalates.
```
...
- HNSW maintenance worker stuck → recall quality slowly drops; alerts.
...
Under heavy load, the substrate may return Overloaded for some operations:

- Writes shed first (less critical for reads).
- Cross-shard operations shed before single-shard.
- Background work pauses.

Clients see Overloaded for shed operations. This is partial-success: the substrate is still operational, but capacity is limited.
...
The embedder's failure doesn't take down the substrate.
...
- Internal substrate operations (within process) continue.
...
| Region (v2) | All shards in this region |
...
| Substrate process crash | Auto-restart by supervisor + WAL replay |
...
When something fails, only its dependents fail. Most of the substrate keeps working.
...
This prevents one dependency from cascading into substrate-wide issues.
...
The application doesn't need to model partial substrate state — just handle errors per operation.
