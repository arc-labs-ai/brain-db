...
For real-time tier, Brain v1 isn't sufficient (no replication). v2's replication targets this.
...
For automation, configure the substrate's snapshot worker with an off-site uploader. The substrate doesn't have direct cloud-storage integration in v1; standard tools handle it.
...
3. Verify the restored substrate works.
...
## 9. The "secondary substrate" approach

Run a secondary substrate in a different region:
...
For full real-time replication, v2's replication will be needed.

## 10. The data-only DR

Sometimes only data needs DR; the substrate code can be redeployed quickly:

- Data files (arena, WAL, metadata) are backed up.
- The substrate binary is in version control / artifact storage.

In a disaster:
- Spin up new infrastructure.
- Deploy the substrate version.
- Restore data.
- Resume.
...
## 20. The "what we can't recover" honest list
...
The substrate's design pushes the "lost" category to be small, but it's not zero. Operators should set expectations accordingly.
