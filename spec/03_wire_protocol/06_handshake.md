...
struct HelloPayload {
    ...
    supported_versions: Vec<u8>,    // protocol versions the client supports, e.g., [1, 2]
    capabilities: HelloCapabilities,
    client_session_token: Option<[u8; 32]>,  // for resumption (not used in v1)
}

struct HelloCapabilities {
    streaming: bool,                 // always true in v1
    compression_zstd: bool,          // not used in v1; reserved
    server_push: bool,               // not used in v1; reserved
}
```
...
In v1, this is `[1]`. As newer protocol versions are introduced, clients list both old and new.
...
In v1, the only required capability is `streaming = true`.

### 2.4 client_session_token

Reserved for future session-resumption (post-failover or after brief disconnects). Not used in v1; clients send `None`.
...
- Future session resumption (not used in v1).
...
The agent's permitted operations. Typically all cognitive operations are allowed; admin operations require elevated permissions.
...
In v1, resumption is not implemented. Disconnections require full re-handshake on reconnect. This is a v2 feature.

## 9. Multi-AUTH

Some applications would benefit from changing identity within a connection — e.g., a proxy serving many users wants to multiplex their identities over one underlying connection. v1 does not support this; one AUTH per connection.

For multi-tenant proxies, the recommended pattern is one connection per identity (subject to connection-pool limits). Multi-AUTH is an open question for v2.
...
*Continue to [`07_request_frames.md`](07_request_frames.md) for per-opcode request layouts.*
