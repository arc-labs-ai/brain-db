...
- **Server-initiated streams** are reserved for **even** IDs: 2, 4, 6, ... — not used in v1.
...
- If the client lags, the server stops sending events; queued events build up server-side until either the client catches up or the queue exceeds an internal limit (and events get dropped — see [09. Cognitive Operations](../09_cognitive_operations/) §SUBSCRIBE for the lossy-vs-lossless modes).
...
*Continue to [`10_errors.md`](10_errors.md) for error handling.*
