...
The same glossary is hosted in [`../00_master_overview/01_glossary.md`](../00_master_overview/) for cross-spec reference; the two files MUST stay in sync.
...
**Brain.** This system. The cognitive substrate.
...
**Cognitive operation.** One of the five primitives (`ENCODE`, `RECALL`, `PLAN`, `REASON`, `FORGET`) plus the supporting operations. The substrate's external interface.
...
**Consolidation.** The background process that compresses similar episodic memories into semantic summaries. The "sleep" analogue from cognitive science. Detailed in [11. Background Workers](../11_background_workers/).
...
**Cue.** The input to `RECALL`, `PLAN`, or `REASON`. Typically text that the substrate embeds into a query vector.
...
**Embedding.** The act of converting text to a vector. The result is a 384-dim `f32` vector (for `bge-small-en-v1.5`). The substrate owns embedding; clients send text, vectors are internal.

**Embedding layer.** Layer L2 of the architecture. Owns the embedding model, tokenizer, inference, batching, and caching. Detailed in [04. Embedding Layer](../04_embedding_layer/).

**Epoch.** A logical unit of time in the lock-free reclamation system. Readers acquire an epoch handle while traversing shared structures; writers wait for old epochs to drain before reclaiming memory. See [10. Concurrency + Epoch Model](../10_concurrency_epochs/).
...
**Eviction.** The removal of a memory from the substrate via background work (not via explicit `FORGET`). Eligibility is determined by salience and capacity pressure.
...
**Idempotency horizon.** The time window during which a `request_id` is retained for deduplication. Default 5 minutes. Retries within the horizon are deduplicated; retries beyond it may be treated as new operations.
...
**Metadata store.** The redb-backed B-tree holding non-vector memory data: salience, context, timestamps, edge lists, contexts. Per-shard. See [07. Metadata + Graph Store](../07_metadata_graph/).
...
**Shard.** A unit of horizontal scaling. One agent's data lives in exactly one shard. Identified by 16-bit runtime `shard_id` (in MemoryIds) and a stable UUIDv7 `shard_uuid` (in storage). See [12. Sharding + Clustering](../12_sharding_clustering/).
...
**Substrate.** Brain. The underlying system that provides cognitive operations to agents.
...
**Transaction (`txn`).** A grouping of multiple operations that commit atomically. Identified by `txn_id`. Supports rollback before commit. See [09. Cognitive Operations](../09_cognitive_operations/) §Transactions.
...
**WAL (Write-Ahead Log).** Per-shard append-only log of state mutations. The durability barrier for `ENCODE` and `FORGET`. Source of truth for crash recovery. See [05. Storage: Arena & WAL](../05_storage_arena_wal/).
...
*Continue to [`10_open_questions.md`](10_open_questions.md) for unresolved architectural questions.*
