# 12.08 Sharding & Clustering Failure Modes

What can go wrong with sharding and (in v2) clustering, and how the substrate responds.

## 1. Single-shard process crash

**Failure mode.** The substrate process crashes; all shards go offline.
...
**Response.** Returns `WrongShard` error with the correct shard ID.
...
**Failure mode.** The substrate is configured with N shards but the data directory has fewer / more.

**Detection.** Startup mismatch.

**Response.** The substrate refuses to start. Logs an error.
...
**Detection.** The router resolves to an unknown shard.
...
**Response.** Shard fails to start. The substrate may continue with other shards or fail entirely.

**Operator action.** Adjust CPU pinning configuration.

## 12. Cross-node call timeout (v2)
...
## 18. The "v1 to v2 upgrade" failure

**Failure mode.** Upgrading from v1 to v2 (single-node to clustered) hits issues.

**Detection.** Migration scripts fail.

**Response.** Stay on v1; address issues; retry.
...
- The substrate keeps the source's data (the destination's partial copy is discarded).
