# 09.10 Admin Operations

The substrate's administrative primitives — operational, not agent-facing.
...
Detailed in [05.10 Snapshots](../05_storage_arena_wal/10_snapshots.md).
...
For long-running admin operations, the substrate returns a job ID; the client polls for completion.

## 19. The "no admin in production" anti-pattern

Some admin operations (e.g., FLUSH_CACHES, REBUILD_ANN) are useful in development but risky in production. The substrate's deployment configuration can disable them:
...
We've avoided putting agent-facing semantics in admin (e.g., we don't have `ADMIN_RECALL_FROM_OTHER_AGENT`). Admin is about substrate operation, not about cognition.

---

*Continue to [`11_consistency.md`](11_consistency.md) for the consistency model.*
