...
- The substrate ensures the read sees all writes durably committed before the request was issued.
...
- If the request includes the write's `last_lsn` (returned in the encode response), the substrate waits exactly until that LSN is published.
...
We don't provide stronger isolation (snapshot isolation, serializable). The substrate's writer-per-shard discipline obviates many of the problems stronger isolation would solve.
...
The substrate exposes this in the SDK as a "wait for write" helper.
...
For cross-shard ordering questions ("did A on shard X happen before B on shard Y?"), the substrate doesn't have a definitive answer. Within a shard, LSN order is the authority.
...
We could make ReadAfterWrite the connection default — but this would slow down most reads (which don't need it). The per-request flag is more flexible.
...
Operations across agents (rare) have weak consistency. The substrate doesn't coordinate.
...
For v1, the eventual + opt-in ReadAfterWrite is the simplest model that meets typical agent needs. Stronger modes are open questions.
