...
brain.encode("step 2")        // ENCODE 2 fails — now we have step 1 dangling
```
...
On recovery, the substrate sees the records:
...
The substrate doesn't provide stricter isolation (e.g., serializable). For most agent workloads, read-committed is sufficient.
...
- After the duration, the substrate auto-aborts.
...
These limits keep the substrate's per-transaction memory bounded.
...
Rare; reserved for future use if cross-transaction conflicts become possible. In v1, single-writer-per-shard means no conflicts.
...
A transaction can't contain another TXN_BEGIN. The substrate rejects nested transactions.
...
For workflows that touch multiple shards atomically, the substrate doesn't help. Agents that need this:

1. Decompose into single-shard transactions.
2. Use a saga pattern with compensating actions on failure.

Cross-shard transactions are an open question; not in v1.
...
If the client retries TXN_COMMIT (e.g., network drop after commit but before ack), the substrate sees the same TXN_COMMIT request_id and replays the response. No double-commit.
...
For read-after-write within the same transaction (operations following the commit on the same client), the substrate ensures visibility — the next RECALL sees the writes immediately.
...
*Continue to [`09_subscribe.md`](09_subscribe.md) for SUBSCRIBE.*
