...
- The metrics the substrate exposes.
...
This document gives them the tools.

## 2. The "operate via signals" approach

Brain is observed primarily through:

- **Metrics**: numerical time series. For dashboards, alerts, and trends.
- **Logs**: structured event records. For investigation and audit.
- **Traces**: per-request causal chains. For deep debugging.
...
- Metrics in OpenMetrics format (Prometheus-compatible).
...
The substrate observes itself:

- Health endpoints (`/healthz`, `/readyz`).
- Self-diagnostics (`/debug/...`).
- Status page (HTML at `/`).

Operators can check the substrate without external tooling.
...
```
Level 1: Substrate health.
  Is the substrate up and accepting traffic?
...
Level 4: Per-tenant health.
  Are specific agents experiencing issues?
```
...
These three together usually identify the issue. The substrate exposes them readily.
...
- Engineering escalation if the issue is a substrate bug.

The substrate provides debugging facilities; doesn't hide.
...
*Continue to [`01_metrics.md`](01_metrics.md) for metrics.*
