...
The substrate starts replay from the `durable_lsn` of the most recent valid checkpoint. The checkpoint is in the metadata store ([`09_checkpointing.md`](09_checkpointing.md)) and includes:
...
The substrate enumerates `wal/*.wal` segments. The segment containing `durable_lsn + 1` is identified by its starting LSN (in the segment header).
...
- The substrate may detect inconsistency and re-run recovery on a subset.
...
A partial transaction at the end of the WAL is treated as TXN_ABORT — the operation didn't complete, so we don't apply it.

## 7. Verifying integrity

After replay completes, the substrate optionally runs integrity checks:
...
If a check fails, the substrate logs the inconsistency. Depending on severity:
...
After WAL replay, the substrate:
...
For a 1M-memory shard, HNSW rebuild takes ~30 seconds (single-threaded) or ~5 seconds (parallel). For larger shards, the time scales linearly. See [06. ANN Index](../06_ann_index/) §Rebuild for details.
...
The substrate logs the LSN of the corrupted record and refuses to proceed. Operators must investigate.

A future enhancement would be a recovery mode that skips the corrupted record and continues — but this risks silently dropping operations, so we don't enable it by default.
...
Slot-level corruption is detected lazily (on access or scrubbing) — the substrate doesn't full-scan the arena at startup (too expensive).
...
2. The substrate starts up, finds the restored files, and runs recovery.
...
The substrate exposes recovery progress via:
...
When recovery completes, the substrate is in a state equivalent to one that:
...
*Continue to [`09_checkpointing.md`](09_checkpointing.md) for checkpoints.*
