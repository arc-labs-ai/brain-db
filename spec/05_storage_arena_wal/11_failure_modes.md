# 05.11 Storage Layer Failure Modes

What can go wrong in the storage layer and the substrate's response. Failures that span multiple layers are documented in [15. Failure Modes + Recovery](../15_failure_recovery/); this file covers the storage-specific cases.
...
- The substrate retries the next attempt; if it succeeds, normal operation resumes.
...
- The substrate marks the WAL as "broken".
...
**Detection.** The substrate doesn't directly detect this; it manifests as elevated read latency (page faults to disk on every search).
...
## 4. Arena partial write before fsync (we don't fsync arena)

**Failure mode.** The arena is being written by `memcpy` into the mmap region. The substrate crashes before the kernel writes the dirty pages back.
...
- The substrate may attempt repair: re-embed the text from the metadata store. If the model is still available and the text is intact, repair succeeds.
...
**Detection.** At reclaim time, the substrate checks if the version would overflow.
...
**Detection.** At the next WAL append after the deletion, the substrate notices the file is gone.

**Response.** The substrate logs the error and treats the WAL as broken. Recovery on next restart will detect the gap (segments aren't consecutive).
...
**Detection.** At the next WAL append (if it falls in the truncated region), the substrate notices the offset doesn't match expected.

**Response.** Same as deletion — WAL marked broken.

**Implication.** The truncation may have happened to the active segment. Records that the substrate believed were durable may now be lost. This is a serious incident.
...
**Response.** The substrate refuses to start. Recovery cannot proceed without the metadata store.
...
**Response.** The substrate logs the error and treats the affected files as inaccessible. The shard may transition to read-only or refuse to operate at all, depending on which file is affected.
...
**Implication.** Use storage with non-volatile write caches (battery-backed, supercap-backed) for production. Without such storage, the substrate's durability is "best effort" — it promises what the kernel promises, which depends on the device.

## 13. Concurrent open of arena from multiple processes

**Failure mode.** Two substrate processes (or other processes) attempt to open the same arena.

**Detection.** The substrate uses a per-shard lock file (`shard.lock`) to enforce single-writer.

**Response.** The second process refuses to start, logging the conflict.

**Operator action.** Ensure only one substrate process has access to a given shard's data directory. This is typical for any embedded database.
...
**Detection.** Implementation-level: the substrate uses `arc-swap` to publish the current mmap, and readers always reload it for each access.
...
**Implication.** None operationally; this is internal correctness handled by the substrate.

## 15. Arena read past end of file

**Failure mode.** A bug somewhere in the substrate causes a read at an offset beyond the arena file's allocated size.
...
**Response.** The substrate's panic handler catches the SIGBUS and logs the offending address. The process may abort.

**Operator action.** This is a bug; report it. The substrate should not produce out-of-bounds reads in practice.
...
**Response.** The substrate refuses to restore the snapshot. Operator must use a different snapshot.
...
**Response.** None automatic. The substrate trusts the kernel's documented semantics.
...
*Continue to [`12_open_questions.md`](12_open_questions.md) for unresolved questions.*
