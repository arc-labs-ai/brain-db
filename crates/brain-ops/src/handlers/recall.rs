...
/// Minimum grounded match score for CURATED mode. Higher than the plain boost
/// floor ([`GROUNDED_MATCH_FLOOR`]): curation REPLACES the associative pile with
/// the grounded shape, so it must be trustworthy. Too low re-creates the
/// grounded-first regression on episodic corpora (a weak/wrong grounded match
/// masks the fan-out's real answer); too high under-curates and returns the pile
/// when a precise answer existed. The single tuning knob for the curate-vs-search
/// boundary; validate against episodic recall before lowering.
const CURATION_MATCH_FLOOR: f32 = 0.5;

fn curation_confident(answer: &GroundedAnswer) -> bool {
    answer
        .values
        .first()
        .map(|v| v.match_score >= CURATION_MATCH_FLOOR)
        .unwrap_or(false)
}

/// Corroboration gate for curation: curate to the grounded shape ONLY when the
/// grounded answer agrees with the associative fan-out — i.e. at least one of its
/// source memories was independently surfaced by the semantic/lexical/graph
/// lanes. Two independent retrieval paths converging on the same memory is strong
/// evidence the grounded answer is right.
///
/// When they DISAGREE — the grounded walk picked a memory the fan-out never
/// surfaced — that is precisely the "grounded is guessing" case (a deep/ambiguous
/// multi-hop where the single cue embedding can't tell the constraint relation
/// from the question relation). There we must NOT curate: returning a confident
/// wrong `Single` is worse than the associative list, where the real answer is
/// still recoverable. This is the safety net that keeps clean single-hop facts
/// curated while refusing to confidently answer what the engine isn't sure of.
fn grounded_corroborated(answer: &GroundedAnswer, associative: &[MemoryResult]) -> bool {
    let assoc: HashSet<u128> = associative.iter().map(|m| m.memory_id).collect();
    answer
        .values
        .iter()
        .filter_map(|v| v.source_memory)
        .any(|mid| assoc.contains(&mid.raw()))
}

/// Build a CURATED response: only the grounded answer's source memories, shaped
/// by its kind (`Single` → one memory, `Set` → its members) — no associative
/// pile. Returns `None` when the answer carries no surfaceable / visible source
/// memory, so the caller falls back to the associative list. The grounded source
/// memories pass through [`hydrate_memories_by_id`], which applies the same
/// agent/kind/salience/tombstone filters as the fan-out, so curation never leaks
/// a memory the caller couldn't otherwise see.
fn curated_frame(
    answer: &GroundedAnswer,
    req: &RecallRequest,
    ctx: &OpsContext,
) -> Result<Option<RecallResponseFrame>, OpError> {
    let mut ids: Vec<MemoryId> = Vec::new();
    let mut seen: HashSet<u128> = HashSet::new();
    for v in &answer.values {
        if let Some(mid) = v.source_memory {
            if mid.raw() != 0 && seen.insert(mid.raw()) {
                ids.push(mid);
            }
        }
    }
    ...
    let answer_kind = match answer.kind {
        AnswerKind::Single => AnswerKindWire::Single,
        AnswerKind::Set => AnswerKindWire::Many,
        AnswerKind::None => return Ok(None),
    };
    let rtxn = ctx
        .executor
        .metadata
        .read_txn()
        .map_err(|e| OpError::Internal(format!("recall curated read_txn: {e}")))?;
    let memories = hydrate_memories_by_id(&rtxn, &ids, req, ctx)?;
    if memories.is_empty() {
        return Ok(None);
    }
    let cumulative_count = u32::try_from(memories.len()).unwrap_or(u32::MAX);
    Ok(Some(RecallResponseFrame {
        answer_kind,
        memories,
        is_final: true,
        cumulative_count,
...
/// Boost a grounded answer's source memories to the top of the combined
/// results without dropping anything. Source memories already present are
/// reordered to the front (grounded order); any source memory the fan-out
/// missed is hydrated and prepended, so the typed graph ADDS recall rather
/// than only reordering it. Returns the full list — the precise answer first,
/// the associative results beneath it.
fn apply_grounded_boost(
    memories: Vec<MemoryResult>,
    answer: &GroundedAnswer,
    req: &RecallRequest,
    ctx: &OpsContext,
) -> Vec<MemoryResult> {
    // Dedup grounded source memories, preserving the confidence-descending
    // order the grounded values already carry.
    let mut source_ids: Vec<MemoryId> = Vec::new();
    let mut seen_src: HashSet<u128> = HashSet::new();
    for v in &answer.values {
        if let Some(mid) = v.source_memory {
            if mid.raw() != 0 && seen_src.insert(mid.raw()) {
                source_ids.push(mid);
            }
        }
    }
    ...
    // Partition the fan-out into the grounded sources (kept in grounded order)
    // and the rest (kept in fan-out order).
    let mut boosted: Vec<MemoryResult> = Vec::new();
    for mid in &source_ids {
        if let Some(m) = memories.iter().find(|m| m.memory_id == mid.raw()) {
            boosted.push(m.clone());
        }
    }
    let present: HashSet<u128> = boosted.iter().map(|m| m.memory_id).collect();

    // Hydrate any grounded source the fan-out didn't surface, and prepend it —
    // the typed graph reaching a memory similarity missed is the whole point.
    let missing: Vec<MemoryId> = source_ids
        .iter()
        .copied()
        .filter(|mid| !present.contains(&mid.raw()))
        .collect();
    let mut hydrated_extra = 0usize;
    if !missing.is_empty() {
        if let Ok(rtxn) = ctx.executor.metadata.read_txn() {
            if let Ok(extra) = hydrate_memories_by_id(&rtxn, &missing, req, ctx) {
                hydrated_extra = extra.len();
                // Grounded-first: prepend before the fan-out hits.
                let mut front = extra;
                front.append(&mut boosted);
                boosted = front;
            }
        }
    }
    ...
    tracing::debug!(
        ...
        cue = %req.cue_text,
        path = "grounded_boost",
        predicate = answer.values.first().map(|v| v.predicate.as_str()).unwrap_or(""),
        match_score = answer.values.first().map(|v| v.match_score).unwrap_or(0.0),
        boosted = boosted.len(),
        hydrated_extra,
        "recall: typed-graph grounded answer boosted to front (non-displacing)"
    );

    // Append the remaining fan-out hits beneath the boosted block.
    let mut out = boosted;
    for m in memories {
        if !present.contains(&m.memory_id) {
            out.push(m);
        }
    }
