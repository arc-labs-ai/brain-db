//! Retrieval query exit integration test.
//!
//! Drives the full retrieval query pipeline end-to-end through the
...
//! - **Substrate RECALL** transparently routes through the retrieval
//!   pipeline on schema-declared deployments.
//!
//! Complements the per-op wire tests
//! (`query_wire.rs`, `recall_routing.rs`) — those
...
/// / p99 2 ms.
#[tokio::test(flavor = "current_thread")]
async fn retrieval_explain_renders_plan_text() {
    let server = start(1).await;
    let mut client = TcpStream::connect(server.data_plane_addr)
        .await
        .expect("connect");
...
/// plan text.
#[tokio::test(flavor = "current_thread")]
async fn retrieval_trace_includes_execution_block() {
    let server = start(1).await;
    let mut client = TcpStream::connect(server.data_plane_addr)
        .await
        .expect("connect");
...
/// Substrate RECALL transparently routes through the retrieval path
/// when a schema is declared. Returned
/// `MemoryResult`s have `contributing_retrievers` populated and
/// `fused_score > 0`.
#[tokio::test(flavor = "current_thread")]
async fn recall_after_schema_routes_through_retrieval_pipeline() {
    let server = start(1).await;
    let mut client = TcpStream::connect(server.data_plane_addr)
        .await
        .expect("connect");
    ...
    match body {
        ResponseBody::Recall(r) => {
            ...
            assert!(
                ...
                "retrieval path must populate fused_score > 0",
            );
        }
