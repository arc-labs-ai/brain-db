//! Relation-op request payloads. Spec §28/07.
//!
//! Mirrors the value-side `brain_core::knowledge::Relation` /
//! `RelationType` but uses wire-domain primitives so rkyv derives
//! fire without coupling brain-core to rkyv. Conversion lives in
//! [`crate::knowledge::relation_resp`] alongside `RelationView`.
...
/// `RELATION_CREATE` (`0x0150`). Spec §28/07 §3.1.
///
...
/// `RELATION_GET` (`0x0151`). Spec §28/07 §4.
...
/// `RELATION_SUPERSEDE` (`0x0152`). Spec §28/07 §5.
#[derive(Archive, Serialize, Deserialize, Clone, Debug, PartialEq)]
#[archive(check_bytes)]
#[archive_attr(derive(Debug))]
...
/// `RELATION_TOMBSTONE` (`0x0153`). Spec §28/07 §6.
#[derive(Archive, Serialize, Deserialize, Clone, Debug, PartialEq)]
#[archive(check_bytes)]
#[archive_attr(derive(Debug))]
...
/// `RELATION_LIST_FROM` (`0x0154`). Spec §28/07 §7.
///
...
/// `RELATION_LIST_TO` (`0x0155`). Spec §28/07 §8.
///
...
/// `RELATION_TRAVERSE` (`0x0156`). Spec §28/07 §9.
///
