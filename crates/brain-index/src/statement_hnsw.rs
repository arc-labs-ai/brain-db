...
mod tests {
    ...
    /// Build a vector clustered around `one_hot(0)` with a small,
    /// `rank`-graded perturbation so cosine similarity to the query
    /// `one_hot(0)` strictly decreases as `rank` grows. This gives a
    /// deterministic near-neighbour ordering (rank 0 is closest) —
    /// used to control which candidates a fetch returns first so a
    /// tombstone-attrition test can starve the initial fetch window.
    fn near(rank: usize) -> [f32; VECTOR_DIM] {
        let mut v = zeros();
        v[0] = 1.0;
        if rank > 0 {
            // A distinct perturbation index per rank keeps vectors
            // unique; magnitude grows with rank so similarity falls.
            let idx = 1 + (rank % (VECTOR_DIM - 1));
            v[idx] = 0.01 * rank as f32;
        }
        v
    }
    ...
    fn search_widens_past_tombstone_attrition() {
        // 20 statements ranked by similarity to the query; tombstone the
        // 8 closest. A single fixed fetch (k*OVER_FACTOR = 10) would then
        // surface only 2 live neighbours — fewer than k=5. Escalation
        // must widen the fetch and still return k live results.
        let mut idx = StatementHnswIndex::new(StatementHnswParams::default_v1()).unwrap();
        let ids: Vec<StatementId> = (0..20).map(|_| StatementId::new()).collect();
        for (rank, id) in ids.iter().enumerate() {
            idx.insert(*id, &near(rank)).unwrap();
        }
        for id in &ids[0..8] {
            idx.mark_tombstoned(*id).unwrap();
        }

        let r = idx.search(&near(0), 5).unwrap();
    ...
    fn search_exhausts_cleanly_when_fewer_than_k_live() {
        ...
        // survivors and terminate (no infinite escalation loop).
        let mut idx = StatementHnswIndex::new(StatementHnswParams::default_v1()).unwrap();
        let ids: Vec<StatementId> = (0..6).map(|_| StatementId::new()).collect();
        for (rank, id) in ids.iter().enumerate() {
            idx.insert(*id, &near(rank)).unwrap();
        }
        for id in &ids[0..3] {
