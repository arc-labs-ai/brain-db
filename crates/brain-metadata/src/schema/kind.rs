...
const BUILTIN_KIND_GUIDE: [(&str, &str, &str, &str); 6] = [
    (
        ...
        "single",
        "state",
        "a single-valued current property of ONE entity (its city, role, age, name, status) — a new value replaces the old",
    ),
    (
        "Relation",
        "set",
        "state",
        "a link to ANOTHER entity (works_at, knows, married_to, has_child, located_in) — accumulates as a set",
    ),
    (
        "Preference",
        "set",
        "state",
        "the subject likes / wants / avoids something — carry the +/- polarity (a dislike is NOT a like)",
    ),
    (
        "Event",
        "set",
        "event",
        "something that HAPPENED at/over a time — an action, occurrence, or change (ran, moved, met, was born, graduated); takes event_at. Prefer Event over Fact for any action or occurrence",
    ),
    (
        "Directive",
        "single",
        "state",
        "a standing behavior rule for/about the subject (response style, standing instruction) — one rule per key",
    ),
    (
        "Fact",
        "set",
        "atemporal",
        "use ONLY when none of the above fits — the true fallback, not the default",
    ),
];
